...
<div class="max-w-2xl mx-auto mt-10 bg-white p-6 rounded shadow">
    ...
    <form action="{{ route('admin.majors.store') }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="desc" value="{{ old('desc') }}" class="w-full border rounded p-2" required>
        </div>
        ...
        <div class="mb-4">
            ...
            <select name="tahun_ajaran_id" class="w-full border rounded p-2" required>
                ...
                @foreach($schoolYears as $schoolYear)
                    <option value="{{ $schoolYear->id }}" {{ old('tahun_ajaran_id') == $schoolYear->id ? 'selected' : '' }}>
                        {{ $schoolYear->year }}
                    </option>
                @endforeach
            </select>
