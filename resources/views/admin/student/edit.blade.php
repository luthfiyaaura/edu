    ...
    <form action="{{ route('admin.student.update', $student->id) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <select id="school_year" name="tahun_ajaran_id" class="w-full border rounded p-2" required>
                <option value="" disabled {{ old('tahun_ajaran_id', $student->tahun_ajaran_id) ? '' : 'selected' }}>-- Pilih Tahun Ajaran --</option>
                @foreach ($schoolYears as $year)
                    <option value="{{ $year->id }}" {{ old('tahun_ajaran_id', $student->tahun_ajaran_id) == $year->id ? 'selected' : '' }}>
                        {{ $year->tahun }}
                    </option>
                @endforeach
            </select>
...
<script>
    $(document).ready(function() {
        let oldYear = '{{ old("tahun_ajaran_id", $student->tahun_ajaran_id) }}';
        let oldMajor = '{{ old("jurusan", $student->jurusan) }}';
